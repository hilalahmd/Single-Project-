  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={() => setModalOpen(true)} className="px-6 py-3 bg-[#F97316] hover:bg-[#EA580C] text-white rounded-xl text-[14px] font-bold transition-all shadow-[0_4px_12px_rgba(249,115,22,0.3)]">Book New Session</button>
      </div>
                ...
                <div key={d} className="p-1 border-r border-white/10 last:border-0 relative hover:bg-white/5 transition-colors cursor-pointer group">
                  {t === '18:00' && i === 2 && (
                    <div className="absolute inset-2 bg-[#F97316] text-white rounded-lg p-2 overflow-hidden z-10 shadow-lg border border-orange-400/50 transition-all hover:scale-[1.02] shadow-[0_4px_12px_rgba(249,115,22,0.3)]">
                      ...
                      <p className="text-[10px] text-orange-100 mt-0.5">18:00 - 19:00</p>
                  ...
                  {t === '08:00' && i === 5 && (
                    <div className="absolute inset-2 bg-[#F97316] text-white rounded-lg p-2 overflow-hidden z-10 shadow-lg border border-orange-400/50 transition-all hover:scale-[1.02] shadow-[0_4px_12px_rgba(249,115,22,0.3)]">
                      <p className="text-[12px] font-semibold truncate">Arjun Menon</p>
                      <p className="text-[10px] text-orange-100 mt-0.5">08:00 - 09:00</p>
                      <div className="absolute bottom-2 right-2"><Video size={12} className="text-white" /></div>
                    </div>
                  )}
          ...
          {[
            ...
            <Card key={i}>
              ...
              <div className="flex gap-3">
                <button className="flex-1 flex items-center justify-center px-4 py-2.5 bg-[#F97316] hover:bg-[#EA580C] text-white rounded-lg text-[14px] font-bold transition-all shadow-[0_4px_12px_rgba(249,115,22,0.3)]"><Video size={16} className="mr-2" /> Join Call</button>
                <button className="flex-1 px-4 py-2.5 border border-white/10 text-white hover:bg-white/10 rounded-lg text-[14px] font-bold transition-colors">Reschedule</button>
                <button className="px-4 py-2.5 border border-red-500/30 text-red-400 hover:bg-red-500/10 rounded-lg text-[14px] font-bold transition-colors">Cancel</button>
              </div>
            </Card>
          ))}
        </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Book New Session">
        <div className="space-y-6">
          <div>
            ...
            <select className="w-full px-4 py-3 border border-white/10 rounded-lg text-[14px] bg-white/5 text-white focus:outline-none focus:border-[#F97316] focus:ring-1 focus:ring-[#F97316] appearance-none">
              <option className="bg-[#1E293B]">Arjun Menon (Active Plan)</option>
            </select>
          </div>
          <div>
            <label className="block text-[14px] font-bold text-gray-300 mb-2">Select Date</label>
            <input type="date" className="w-full px-4 py-3 border border-white/10 rounded-lg text-[14px] bg-white/5 text-white focus:outline-none focus:border-[#F97316] focus:ring-1 focus:ring-[#F97316]" />
          </div>
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {['08:00 AM', '10:00 AM', '02:00 PM', '04:00 PM', '06:00 PM', '08:00 PM'].map(t => (
                <button key={t} className="p-3 border border-white/10 bg-white/5 rounded-lg text-[12px] font-bold text-gray-400 hover:border-[#F97316] hover:text-[#F97316] hover:bg-[#F97316]/10 transition-colors">
                  {t}
                </button>
              ))}
            </div>
          ...
          <div className="pt-6 border-t border-white/10 flex justify-end gap-3">
            <button onClick={() => setModalOpen(false)} className="px-6 py-3 border border-white/10 text-white hover:bg-white/10 rounded-lg text-[14px] font-bold transition-colors">Cancel</button>
            <button onClick={() => setModalOpen(false)} className="px-6 py-3 bg-[#F97316] hover:bg-[#EA580C] text-white rounded-lg text-[14px] font-bold transition-all shadow-[0_4px_12px_rgba(249,115,22,0.3)]">Confirm Booking</button>
          </div>
        </div>
      </Modal>
    </div>
