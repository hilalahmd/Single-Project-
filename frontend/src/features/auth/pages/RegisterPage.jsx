      ...
      {Array.from({ length: total }, (_, i) => {
        ...
        return (
          <div key={step} className="flex items-center">
            ...
            : isActive ? 'bg-[#1E293B] border-blue-500 text-blue-400 shadow-[0_0_10px_rgba(37,99,235,0.2)]'
                       : 'bg-[#0F172A] border-[#1E293B] text-gray-500'
            }`}>
              {isDone ? <Check size={14} strokeWidth={3} /> : step}
            </div>
            {i < total - 1 && (
              <div className={`w-12 h-0.5 transition-all duration-300 ${step < current ? 'bg-blue-600' : 'bg-[#1E293B]'}`} />
            )}
          </div>
        )
      })}
    ...
    <div className="min-h-screen bg-[#07080C] flex flex-col text-white font-['Inter'] selection:bg-[#2563EB] selection:text-white relative overflow-x-hidden">
      ...
      <header className="relative z-50 w-full pt-8 px-6 sm:px-12 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group">
          ...
          <span className="text-blue-400 font-bold text-[10px] tracking-widest uppercase ml-2 border border-blue-500/30 px-2 py-0.5 rounded-md bg-blue-500/10 hidden sm:block shadow-[0_0_10px_rgba(37,99,235,0.2)]">Sign Up</span>
        </Link>
        <Link to="/auth/login" className="text-sm font-bold text-gray-400 hover:text-white transition-colors border-b border-transparent hover:border-white pb-0.5">
          Sign in &rarr;
        </Link>
      </header>
  ...
  return (
    <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left relative z-10">
      <div className="inline-flex items-center gap-2 bg-[#1E293B]/40 border border-[#2563EB]/30 rounded-full px-4 py-2 mb-8 shadow-[0_0_20px_rgba(37,99,235,0.15)] backdrop-blur-md">
        ...
        <span className="text-blue-300 text-xs font-bold tracking-widest uppercase">Join FitForge</span>
      </div>
      <h1 className="text-5xl sm:text-6xl font-black text-white font-['Syne'] leading-[1.1] mb-6 tracking-tight drop-shadow-2xl">
        Start your{' '}
        <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-blue-600">transformation.</span>
      </h1>
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row items-center justify-center px-4 sm:px-8 py-12 z-10 relative max-w-7xl mx-auto w-full gap-16">
      ...
      <div className="w-full lg:w-5/12 max-w-md relative z-10">
        ...
        <div className="bg-[#0F172A]/60 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 sm:p-10 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
...
export default function RegisterPage() {
  ...
  // ── Input style helpers ──────────────────────────────────────────────────────
  const inputCls = "w-full px-5 py-4 text-sm focus:outline-none bg-transparent text-white placeholder-gray-600 font-medium"
  const fieldWrap = "relative flex rounded-2xl overflow-hidden border border-[#1E293B] bg-[#030712]/80 backdrop-blur-sm focus-within:border-blue-500/50 transition-colors"
  const labelCls = "block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 ml-1"
  ...
  const btnBack = "flex-1 py-4 bg-[#0F172A] border border-[#1E293B] text-white font-bold rounded-2xl hover:bg-[#1E293B] transition-all"
      ...
      <PageWrapper>
        <div className="flex-1 flex items-center justify-center px-4 z-10 relative">
          <div className="w-full max-w-md relative">
            <div className="absolute inset-0 bg-gradient-to-tr from-blue-600/20 via-transparent to-indigo-600/20 rounded-3xl blur-2xl transform rotate-3 scale-105 -z-10 pointer-events-none" />
            <div className="bg-[#0F172A]/60 backdrop-blur-2xl border border-white/10 rounded-3xl p-12 shadow-[0_8px_32px_rgba(0,0,0,0.5)] text-center space-y-6">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white font-['Syne'] tracking-tight">Account Created!</h2>
  ...
  return (
    <PageWrapper>
      ...
      {step === 1 && (
        <Card step={step}>
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-white font-['Syne'] tracking-tight">Create account</h2>
            <p className="text-gray-400 text-sm font-medium mt-1">Step 1 of 4 — Account details</p>
          </div>
          ...
          <div className="text-center space-y-2 pt-4">
            <p className="text-xs text-gray-500 font-medium">
              ...
              <Link to="/auth/login" className="text-blue-400 font-bold hover:text-blue-300 transition-colors ml-1">Sign in</Link>
            </p>
            <p className="text-xs text-gray-500 font-medium">
              Are you a fitness professional?{' '}
              <Link to="/auth/trainer-register" className="text-gray-300 font-bold hover:text-white transition-colors ml-1 border-b border-transparent hover:border-white pb-0.5">Apply as Trainer</Link>
            </p>
          </div>
        </Card>
      )}
      ...
      {step === 2 && (
        <Card step={step}>
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-white font-['Syne'] tracking-tight">Verify your email</h2>
            <p className="text-gray-400 text-sm font-medium mt-1">Step 2 of 4 — Code sent to <span className="text-white font-bold">{form1.email}</span></p>
          </div>
              ...
              {otp.map((digit, i) => (
                <input
                  ...
                  className={`w-12 h-12 text-center text-xl font-bold rounded-xl border transition-all focus:outline-none ${
                    digit ? 'bg-blue-600/10 border-blue-500 text-blue-400 shadow-[0_0_10px_rgba(37,99,235,0.2)]' : 'bg-[#0F172A]/80 border-[#1E293B] text-white focus:border-blue-500/50'
                  }`}
                />
              ))}
            </div>
      ...
      {step === 3 && (
        <Card step={step}>
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-white font-['Syne'] tracking-tight">Your profile</h2>
            <p className="text-gray-400 text-sm font-medium mt-1">Step 3 of 4 — Preferences</p>
          </div>
            ...
            <div>
              ...
              <div className={fieldWrap}>
                <select value={language} onChange={e => setLanguage(e.target.value)} className={`${inputCls} appearance-none`}>
                  <option value="" className="bg-[#0F172A]">Select language</option>
                  {['Malayalam','Hindi','Tamil','Kannada','Telugu','English'].map(l => <option key={l} value={l} className="bg-[#0F172A]">{l}</option>)}
                </select>
              </div>
            </div>
                ...
                {GOALS.map(({ id, label, icon: Icon }) => {
                  ...
                  return (
                    <button key={id} type="button" onClick={() => setSelectedGoal(id)}
                      className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all duration-300 ${active ? 'border-blue-500 bg-blue-600/10 shadow-[0_0_15px_rgba(37,99,235,0.2)]' : 'border-[#1E293B] bg-[#0F172A]/80 hover:border-gray-600 hover:bg-[#1E293B]'}`}
                    >
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/50' : 'bg-[#1E293B] text-gray-400'}`}>
                        <Icon size={18} />
                      </div>
                      <span className={`text-xs font-bold ${active ? 'text-blue-400' : 'text-gray-300'}`}>{label}</span>
                    </button>
                  )
                })}
              </div>
                ...
                {EXPERIENCE.map(lvl => {
                  ...
                  return (
                    <button key={lvl} type="button" onClick={() => setExperience(lvl)}
                      className={`py-3 rounded-xl text-sm font-bold border transition-all duration-300 ${active ? 'border-blue-500 bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.4)]' : 'border-[#1E293B] bg-[#0F172A]/80 text-gray-400 hover:text-white hover:border-gray-600 hover:bg-[#1E293B]'}`}
                    >
                      {lvl}
                    </button>
                  )
      ...
      {step === 4 && (
        <Card step={step}>
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-white font-['Syne'] tracking-tight">Body metrics</h2>
            <p className="text-gray-400 text-sm font-medium mt-1">Step 4 of 4 — Physical details</p>
          </div>
            ...
            <div>
              ...
              <div className={fieldWrap}>
                <select value={form3.activityLevel} onChange={f3('activityLevel')} className={`${inputCls} appearance-none`}>
                  <option value="" className="bg-[#0F172A]">Select activity level</option>
                  <option value="sedentary" className="bg-[#0F172A]">Sedentary (little or no exercise)</option>
                  <option value="lightly-active" className="bg-[#0F172A]">Lightly Active (1–3 days/week)</option>
                  <option value="moderately-active" className="bg-[#0F172A]">Moderately Active (3–5 days/week)</option>
                  <option value="very-active" className="bg-[#0F172A]">Very Active (6–7 days/week)</option>
                </select>
              </div>
            </div>
          </div>
