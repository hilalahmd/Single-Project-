                  ...
                  <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center text-xs font-bold text-black shrink-0 border border-gray-200">{row.name[0]}</div>
                        <div>
                    ...
                    <td className="px-6 py-4 font-medium text-gray-600">{row.sub}</td>
                    <td className="px-6 py-4 font-medium text-gray-600">{row.date}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 text-xs font-bold rounded-full ${row.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{row.status}</span>
                    </td>
                    <td className="px-6 py-4">
          ...
          <>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-8">
              <div className="text-center">
                <div className="w-24 h-24 bg-gray-50 rounded-full flex items-center justify-center text-3xl font-black text-black mx-auto mb-4 border border-gray-200">{selectedUser.name[0]}</div>
                ...
                <div className="flex justify-center gap-2">
                  <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-gray-100 border border-gray-200 text-black">{selectedUser.role}</span>
                  <span className={`px-2.5 py-1 text-xs font-bold rounded-full ${selectedUser.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{selectedUser.status}</span>
                </div>
              </div>
              <div>
                <h4 className="font-bold text-sm text-gray-400 uppercase tracking-wider mb-3">Subscription Info</h4>
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                  <div className="flex justify-between items-center mb-2"><span className="font-medium text-gray-500">Current Plan</span><span className="font-bold text-black">{selectedUser.sub}</span></div>
                  <div className="flex justify-between items-center"><span className="font-medium text-gray-500">Joined</span><span className="font-bold text-black">{selectedUser.date}</span></div>
                </div>
              </div>
              <div>
